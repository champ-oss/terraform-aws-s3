...
resource "aws_s3_bucket_policy" "this" {
  count  = (var.enable_custom_policy || var.enable_lb_policy || length(var.s3_update_cross_account_id_arns) != 0 || length(var.datasync_cross_account_id_arn) != 0 || length(var.aws_cross_account_id_arns) != 0) && var.enabled ? 1 : 0
  bucket = aws_s3_bucket.this[0].id
