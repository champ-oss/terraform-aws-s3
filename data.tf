data "aws_iam_policy_document" "combined" {
  ...
  source_policy_documents = compact([
    var.enable_lb_policy && var.enabled ? data.aws_iam_policy_document.lb[0].json : "",
    var.enable_custom_policy && var.enabled ? var.policy : "",
    length(var.aws_cross_account_id_arns) != 0 && var.enabled ? data.aws_iam_policy_document.cross_account[0].json : ""
  ])
}
